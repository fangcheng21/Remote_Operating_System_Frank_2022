...
public:
	Panda(const std::string& franka_address):Robot1(franka_address)
	{
		...
		sub1 = n.subscribe("CartPos", 10, &Panda::Callback,this);		//接收外部节点位置数据的订阅方
	...
	void Callback(const ros_franka::FrankaCartPos &data)				//接收外部数据的回调函数
	{
...
int main(int argc, char** argv)
{
	...
	pthread_t thread;
	//初始化ros节点
	ros::init(argc, argv, "CartPos_Control");

	//实例化,输入IP地址
	Panda pl("192.168.100.63");	

	//初始化数据
	pl.Robot1.robot_CartPos_init();
	...
	//更新为多线程调用	开启控制
	pl.Robot1.start_Pos_pth(&thread);
