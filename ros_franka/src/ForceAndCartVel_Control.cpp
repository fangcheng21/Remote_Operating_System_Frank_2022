...
#include"ros_franka/FrankaForceAndCartVel.h"
...
public:
	Panda(const std::string& franka_address):Robot1(franka_address)
	{
		...
		sub1 = n.subscribe("ForceAndCartVel", 10, &Panda::Callback,this);		//接收外部节点位置数据的订阅方
	...
	void Callback(const ros_franka::FrankaForceAndCartVel &data)				//接收外部数据的回调函数
	{
		...
		for(float i=1.0f; i<=k; i++)
		{
			...
			for (int j = 0; j < 6; j++)
			{
					double tag = (1-i/k)*CartVelArray[j]  + i/k * data.EndVel[j];
           			Robot1.CartVel_1.push(tag);
			}
		}
...
public:
	std::array<double, 6> CartVelArray{};
...
int main(int argc, char** argv)
{
	...
	pthread_t thread;
	//初始化ros节点
	ros::init(argc, argv, "ForceAndCartVel_Control");
	...
	//更新为多线程调用	开启控制
	pl.Robot1.start_ForceAndCarVel_pth(&thread);
