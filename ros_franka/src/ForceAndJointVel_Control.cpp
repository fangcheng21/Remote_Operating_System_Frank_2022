...
public:
	Panda(const std::string& franka_address):Robot1(franka_address)
	{
		...
		sub1 = n.subscribe("ForceAndJointVel", 10, &Panda::Callback,this);		//接收外部节点位置数据的订阅方
	...
	void Callback(const ros_franka::FrankaForceAndJointVel &data)				//接收外部数据的回调函数
	{
		...
		for(float i=1.0f; i<=k; i++)
		{
			...
			for (int j = 0; j < 7; j++)
			{
					double tag = (1-i/k)*JointVelArray[j]  + i/k * data.JointVel[j];
           			Robot1.JointVel_1.push(tag);
			}
		}
		...
		for(int i=0;i<7; i++)
		{
			JointVelArray[i] = data.JointVel[i];
		}
...
public:
	std::array<double, 7> JointVelArray{};
...
int main(int argc, char** argv)
{
	...
	pthread_t thread;
	//初始化ros节点
	ros::init(argc, argv, "ForceAndJointVel_Control");
	...
	//更新为多线程调用	开启控制
	pl.Robot1.start_ForceAndJointVel_pth(&thread);
