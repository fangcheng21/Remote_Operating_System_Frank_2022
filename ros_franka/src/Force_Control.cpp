...
public:
	Panda(const std::string& franka_address):Robot1(franka_address)
	{
		...
		sub1 = n.subscribe("frocePos_1", 10, &Panda::Callback,this);		//接收外部节点位置数据的订阅方
	...
	void Callback(const ros_franka::FrankaForce &data)				//接收外部数据的回调函数
	{
...
int main(int argc, char** argv)
{
	...
	//更新为多线程调用	开启控制
	pl.Robot1.start_Force_pth(&thread);
