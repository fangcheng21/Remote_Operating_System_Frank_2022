...
public:
	Panda(const std::string& franka_address):Robot1(franka_address)
	{
		...
		sub1 = n.subscribe("JointPos", 10, &Panda::Callback,this);		//接收外部节点位置数据的订阅方
	...
	void Callback(const ros_franka::FrankaJointPos &data)				//接收外部数据的回调函数
	{
		...
		for(float i=1.0f; i<=k; i++)
		{
			for (int j = 0; j < 7; j++)
			{
					double tag = (1-i/k)*JointArray[j]  + i/k * data.JointPos[j];
           			Robot1.joint_1.push(tag);
			}
		}

		for(int i=0;i<7; i++)
		{
			JointArray[i] = data.JointPos[i];
		}
...
int main(int argc, char** argv)
{
	...
	pthread_t thread;
	//初始化ros节点
	ros::init(argc, argv, "JointPos_Control");

	//实例化,输入IP地址
	Panda pl("192.168.100.63");	

	//初始化数据
	pl.Robot1.robot_JoinPos_init();
	...
	float FREQ; 
	pl.n.param<float>("/freq",FREQ,1000);		//此处键入频率
	pl.Robot1.setfrequency(FREQ); //接收频率；该数据要与发送数据给机器人的频率要相同
	...
	//更新为多线程调用	开启控制
	pl.Robot1.start_JointPos_pth(&thread);
