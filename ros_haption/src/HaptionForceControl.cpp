...
public:
	Haption()
	{
		...
		sub1 = n.subscribe("HaptionForcePos", 1000, &Haption::Callback,this); //接收外部节点位置数据的订阅方
	...
	void Callback(const ros_haption::HaptionData &data)	//接收外部数据的回调函数
	{
			...
			for(float i=1.0f; i<=k; i++)
			{
				for (int j = 0; j < 6; j++)
				{
					double tag = (1-i/k)*ForceArray[j]  + i/k * data.Force[j];
           			VH.force_1.push(tag);
				}
			}
...
int main(int argc, char** argv)
 {
	...
	//设置控制模式
    //ha.VH.setCommandtype(COMMAND_TYPE_IMPEDANCE);
	virtSetCommandType(ha.VH.M_VC, COMMAND_TYPE_IMPEDANCE);  
