...
public:
	Haption()
	{
		...
		sub1 = n.subscribe("HaptionJointForcePos", 1000, &Haption::Callback,this); //接收外部节点位置数据的订阅方
	...
	void Callback(const ros_haption::HaptionData &data)	//接收外部数据的回调函数
	{
			...
			for(float i=1.0f; i<=k; i++)
			{
				for (int j = 0; j < 6; j++)
				{
					double tag = (1-i/k)*JointForceArray[j]  + i/k * data.Articularforce[j];
           			VH.Jointforce_1.push(tag);
				}
			}

		for(int i=0;i<6; i++)
		{
			JointForceArray[i] = data.Articularforce[i];
		}
...
public:
	std::array<float, 6> JointForceArray{};
	float k;
	ros::NodeHandle n;  
	ros::Subscriber sub1; 
	ros::Subscriber sub2;
...
int main(int argc, char** argv)
 {
	...
	//设置控制模式
    //ha.VH.setcommandtype_articular_impedance();
	virtSetCommandType(ha.VH.M_VC, COMMAND_TYPE_ARTICULAR_IMPEDANCE);  

	ha.VH.setfrequency(1000);

	std::cout  << "   Press Enter to start Force Control...   " << std::endl;
	std::cin.ignore();

	ha.VH.start_ArticularForce_pth();
