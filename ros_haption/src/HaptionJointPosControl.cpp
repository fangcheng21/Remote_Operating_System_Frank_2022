...
public:
	Haption()
	{
		...
		sub1 = n.subscribe("HaptionJointPos", 1000, &Haption::Callback,this); //接收外部节点位置数据的订阅方
	...
	void Callback(const ros_haption::HaptionData &data)	//接收外部数据的回调函数
	{
			...
			for(float i=1.0f; i<=k; i++)
			{
				for (int j = 0; j < 6; j++)
				{
					double tag = (1-i/k)*JointPosArray[j]  + i/k * data.ArticularPosition[j];
           			VH.Jointpos_1.push(tag);
				}
			}

		for(int i=0;i<6; i++)
		{
			JointPosArray[i] = data.ArticularPosition[i];
		}
...
public:
	std::array<float, 6> JointPosArray{};
	float k;
	ros::NodeHandle n;  
	ros::Subscriber sub1; 
	ros::Subscriber sub2;
...
int main(int argc, char** argv)
 {
	...
	//设置控制模式
    //ha.VH.setcommandtype_articular();
	virtSetCommandType(ha.VH.M_VC, COMMAND_TYPE_ARTICULAR);  
